...
export default function SettingsPage() {
  ...
  // Default password hash for "manager123"
  const DEFAULT_PASSWORD_HASH = simpleHash('manager123')

  const [profile, setProfile] = useState<managerProfile>({
    id: 'manager-001',
    fullName: 'manager User',
    email: 'manager@securesystems.com',
    avatar: null,
    role: 'Super manageristrator',
  ...
  const handleSecurityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      localStorage.setItem('manager-security', JSON.stringify(updatedSecurity))
    }
  }
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      document.title = `${profileForm.fullName} - Secure Systems manager`
  ...
  const saveSecurity = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (securityForm.newPassword) {
        const apiRes = await fetch('/api/manager/password', {
          method: 'POST',
  ...
  const saveSystem = (e: React.FormEvent) => {
    ...
    localStorage.setItem('manager-system', JSON.stringify(systemForm))
    // Apply theme immediately to manager dashboard
    const root = document.documentElement
    if (systemForm.theme === 'light') {
      root.classList.remove('dark')
      // Custom light tint for manager dashboard
      root.style.setProperty('--manager-bg', '#f7fafc')
      root.style.setProperty('--manager-card', '#ffffff')
      root.style.setProperty('--manager-text', '#0f172a')
    } else if (systemForm.theme === 'dark') {
      root.classList.add('dark')
      root.style.removeProperty('--manager-bg')
      root.style.removeProperty('--manager-card')
      root.style.removeProperty('--manager-text')
    }
    setMessages({
      type: 'success',
  ...
  const resetToDefaultPassword = () => {
    ...
    localStorage.setItem('manager-security', JSON.stringify(resetSecurity))
    setMessages({ type: 'info', text: 'Password reset to default: manager123' })
    setTimeout(() => setMessages(null), 3000)
  }
          ...
          <div className='space-y-6'>
            ...
            <form onSubmit={saveProfile} className='space-y-6'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-400'>
                ...
                <div>
                  <span className='block mb-1'>{t('manager_last_login')}</span>
                  <span className='text-gray-300'>
              ...
              >
                {isLoading ? t('manager_saving') : t('manager_save_profile')}
              </button>
            </form>
          </div>
        )}
          ...
          <div className='space-y-6'>
            <div className='flex justify-between items-center'>
              ...
              <button
                ...
                title='Reset password to default (manager123)'
              >
                {t('manager_reset_password')}
              </button>
            </div>
            ...
            <form onSubmit={saveSecurity} className='space-y-6'>
              <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                <div className='space-y-4'>
                  <h3 className='text-lg font-medium text-gray-300'>
                    {t('manager_password_management')}
                  </h3>

                  <div>
                    <label className='block mb-2 text-gray-300'>
                      {t('manager_current_password')} *
                    </label>
                    <input
                      type='password'
                  ...
                  <div>
                    <label className='block mb-2 text-gray-300'>
                      {t('manager_confirm_new_password')}
                    </label>
                    <input
                      type='password'
                    ...
                    {securityForm.newPassword &&
                      securityForm.confirmPassword && (
                        ...
                        >
                          {securityForm.newPassword ===
                          securityForm.confirmPassword
                            ? t('manager_passwords_match')
                            : t('manager_passwords_do_not_match')}
                        </p>
                      )}
                  </div>
              ...
              >
                {isLoading
                  ? 'Updating...'
                  : t('manager_update_security_settings')}
              </button>
            </form>
          </div>
        )}
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                <div className='space-y-4'>
                  ...
                  <div className='flex items-center gap-3 p-4 bg-gray-700/50 rounded-lg border border-gray-600/30'>
                    ...
                    <div>
                      <label className='text-gray-200 font-medium'>
                        {t('manager_automatic_backups')}
                      </label>
                      <p className='text-sm text-gray-400 mt-1'>
                        {t('manager_auto_backup_hint')}
                      </p>
                    </div>
                  </div>
